    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/rachai/auth/**").permitAll() // Libera rotas de login/registro
                .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll() // Libera requisições de preflight do CORS
                .anyRequest().authenticated() // Bloqueia todo o resto exigindo o Token JWT válido
            )
            // Insere o filtro customizado de validação de token antes do filtro padrão de usuário/senha
